<!-- Main Sidebar Container -->
<aside class="main-sidebar sidebar-light-primary elevation-4">
    <!-- Brand Logo -->
    <a href="#" class="brand-link navbar-primary">
      {{-- <img src="/lte/dist/img/AdminLTELogo.png" alt="AdminLTE Logo" class="brand-image img-circle elevation-3"
           style="opacity: .8"> --}}
      <span class="brand-text font-weight-light text-white">PDAM BANDARMASIH</span>
    </a>

    <!-- Sidebar -->
    <div class="sidebar">
      <!-- Sidebar Menu -->
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          <!-- Add icons to the links using the .nav-icon class
               with font-awesome or any other icon font library -->
          <li class="nav-item">
            <a href="/home" class="nav-link">
              <i class="nav-icon fas fa-tachometer-alt"></i>
              <p>
                Dashboard
              </p>
            </a>
          </li>
          <li class="nav-item has-treeview menu-open">
            <a href="#" class="nav-link">
              <i class="nav-icon fas fa-circle"></i>
              <p>
                Data Master
                <i class="right fas fa-angle-left"></i>
              </p>
            </a>
            <ul class="nav nav-treeview">
              <li class="nav-item">
                <a href="/departemen" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Departemen</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/teknisi" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Teknisi</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/pengguna" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Pengguna</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/rekanan" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Rekanan</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/infrastruktur" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Infrastruktur</p>
                </a>
              </li>
              
              <li class="nav-item">
                <a href="/user" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>User</p>
                </a>
              </li>
            </ul>
          </li>
          
          <li class="nav-item has-treeview menu-open">
            <a href="#" class="nav-link">
              <i class="nav-icon fas fa-circle"></i>
              <p>
                Transaksi
                <i class="right fas fa-angle-left"></i>
              </p>
            </a>
            <ul class="nav nav-treeview">
            <li class="nav-item">
              <li class="nav-item">
                <a href="/jadwal" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Jadwal Pemeliharaan</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/hasil" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Hasil Pemeliharan</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/serahterima" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Serah Terima</p>
                </a>
              </li>
              <li class="nav-item">
                <a href="/pemusnahan" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Pemusnahan</p>
                </a>
              </li>
              
              {{-- <li class="nav-item">
                <a href="/pengeluaran" class="nav-link">
                  <i class="far fa-circle nav-icon"></i>
                  <p>Beban Pengeluaran</p>
                </a>
              </li> --}}
            </ul>
          </li>
          
          <li class="nav-item">
            <a href="/laporan" class="nav-link">
              <i class="nav-icon fa fa-file"></i>
              <p>
                Laporan
              </p>
            </a>
          </li>
          <li class="nav-item">
            <a href="/logout" class="nav-link">
              <i class="nav-icon fa fa-sign-out-alt"></i>
              <p>
                Logout
              </p>
            </a>
          </li>
        </ul>
      </nav>
      <!-- /.sidebar-menu -->
    </div>
    <!-- /.sidebar -->
  </aside>